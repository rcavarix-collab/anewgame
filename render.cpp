...
static const char* g_shaderSrc =
    ...
    "    float3 ambient = lerp(fAmbientDown.rgb, fAmbientUp.rgb, n.y * 0.5f + 0.5f);\n"
    // Fill light (D63): what the sun lights besides the surfaces its beam
    // reaches. Some of it comes back from the whole sky, and the sky is
    // brightest around the sun, so a face turned toward it gets more than
    // one turned away: a gradient that keeps a hill's
    // shape in its own shadow. And sunlit ground throws a warm bounce onto
    // faces that tilt sideways or down (cliffs, overhangs). Without these,
    // everything in shadow was one flat blue slab under a low sun. Two dot
    // products from constants: no pass, no data.
    "    float toSun = dot(n, fSunDir.xyz) * 0.5f + 0.5f;\n"
    "    float groundLit = sqrt(saturate(fSunDir.y));\n"
    "    ambient += fSunColor.rgb * (0.08f + 0.12f * toSun * toSun + 0.25f * groundLit * saturate(0.5f - 0.5f * n.y));\n"
    // Keep shadow from going deep blue: the sky's light, a quarter greyer.
    "    ambient = lerp(ambient, dot(ambient, float3(0.2126f, 0.7152f, 0.0722f)).xxx, 0.25f) * (ao * sky);\n"
