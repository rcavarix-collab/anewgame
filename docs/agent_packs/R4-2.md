**Part 2 of 2, the last** of walkgrid helper task R4. You now have everything (read it together with the earlier parts): do the task.
...
Do task R4 as the Brief says: `REPORT.md` first, then each file under its `=== FILE:` line, and `END OF DELIVERY` at the very end.
