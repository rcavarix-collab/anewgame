...
- **Research from reliable sources only (D66).** Agencies, standards bodies, peer-reviewed or society publications, recognised references; never forums or social sites. Like a college paper: independent sources agreeing, testable figures, every source listed, anything not read at the source marked. Be a considerate visitor: few requests, no bulk fetching, and a site that refuses access is respected, not worked around. Research is for theory and explanation; never copy anyone's code, art, text or data tables: we write our own from understanding, and a paper's data is written down as a description of what it shows, then matched by our own numbers (D68). Notes go in `docs/research/`. Research is a regular step (D67): every feature card says what the theory offers, the overview (`docs/research/OVERVIEW.md`) is refreshed each milestone, and puzzling play-test problems get a look at the theory before a fix is planned.
