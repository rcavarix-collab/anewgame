...
| **0.3 Docs** | Archive the Voxistics docs, write the new `CLAUDE.md` (**shown to you before commit**), start walkgrid's `DESIGN.md` from the engine sections that carry over | `CLAUDE.md`, `DESIGN.md` → `reference/voxistics/DESIGN.md`, new `DESIGN.md`, `docs/REVIEW_2026-09.md` → `reference/voxistics/` (`docs/SOUND_PALETTE.md` stays: the palette carries over) |
...
| | *Done. Deviation: the chunk-arrived hook was removed rather than made a generic callback, because nothing needs it now; it comes back as a callback when something does. The pulse textures moved to `assets/textures/parked/`, since an unused texture raises a start-up warning.* | |
