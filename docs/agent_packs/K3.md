# walkgrid helper task K3, Built piece catalogue (S)

Everything for this task is in this one message (made 2026-09-26). It is task K3 only: ignore any other walkgrid task you may remember from earlier chats.
...
Do task K3 now: the report under `=== FILE: REPORT.md ===` first, then each file under its `=== FILE:` line, and `END OF DELIVERY` at the very end.
