# walkgrid helper task A9, Gravel (S)

Everything for this task is in this one message (made 2026-09-26). It is task A9 only: ignore any other walkgrid task you may remember from earlier chats.
...
Do task A9 now: the report under `=== FILE: REPORT.md ===` first, then each file under its `=== FILE:` line, and `END OF DELIVERY` at the very end.
