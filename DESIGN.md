...
texTop / texSide / texBottom — the top shows on ground up to ~70°, the side only on banks steeper than that (23.3, D50)
...
### 4.6.2 Text for any language (strtable.h, M1.10, D26)
- **Every player-facing word is in `assets/text/<language>.txt`**, never in code: one `key = text` a line, UTF-8, `{0}`, `{1}`… for the values the game fills in (in whatever order a language needs), `\n` for a line break. English (`en.txt`) loads first and the chosen language over it, so an untranslated key shows English; a key missing from both shows as `[key]`. The language is `language=` in settings.cfg (`en` by default; a file name only, so a hand-edited value can't reach outside the folder). Material names are keyed by their registry name (`material.meadow_grass`); the action names in Keybindings by `action.*`.
- **The font follows the table.** The atlas bakes ASCII, Latin-1, then every other code point the loaded table uses, so a translation's letters are there without the atlas carrying every script; the table's `font` key names the installed system font to draw it with (Consolas for English; GDI+ doesn't fall back between fonts). A letter the atlas lacks draws as `?`. Key names in Keybindings come from Windows in the player's own language, as UTF-8.
- **Layouts measure text.** Row buttons, panel titles and the library's hint shrink to the next smaller baked size until the text fits, so a longer language doesn't spill out of its button.
- **Checked, not trusted.** `tools/check_strings.py` (run by `tests/run.sh`) fails on any word left in a string literal in the game layer or `main.cpp`, on keys used but not defined or defined but not used, and on a translation whose `{n}` slots don't match English. Debug aids (the F3 overlay, the performance report, the F8 clock toast, debugger output) stay in English, inside `// D26: debug text` … `// D26: end` or marked `// D26: not player text`. `--pseudo` writes `assets/text/test.txt`, an accented pseudo-language a third longer than English, for seeing untranslated text and tight layouts (`language=test`).
- **Monospace only, for now.** The atlas is a monospace grid (4.6); a script whose letters are wider than Consolas' advance (Chinese, Japanese, Korean) would need wider cells, which is the planned change when such a language is wanted.
...
- **Fog.** A faint aerial haze at a fixed scale (about 15 % at 80 blocks, 45 % at most; the same at any render distance, D56), and the edge itself still fades fully into the sky, so depth reads and the far terrain never ends in a hard line.
...
### 7.2.1 Save location
`Documents\My Games\walkgrid\`, in the Documents folder Windows gives the player (`FOLDERID_Documents`: the one Explorer shows, wherever it has been moved, and where other games keep theirs), so they can find it (D45). If that Documents is inside OneDrive, the folder is `Saved Games\walkgrid\` instead: never a cloud-synced folder (D28). Subfolders: `Saves`, `ShaderCache`, `Screenshots`; `settings.cfg` and `perf_report.txt` sit in the folder itself. The folder is resolved fresh on every use and created if missing; if the path is blocked (a file where a folder should be) or can't be resolved, the working directory is used instead. Once, if the chosen folder doesn't exist yet and an earlier build's does (`%USERPROFILE%\Documents\My Games\walkgrid`, or `Saved Games\walkgrid`), its contents are moved across. Screenshots and the performance report show their full path when saved, and only once the file is confirmed on disk.
