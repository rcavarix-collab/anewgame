...
| Material blending | Height-based: weights lifted by each texture's height map, tallest wins in a narrow band, world noise | Height blending is common practice (engine documentation and talks) [k]; splat maps; dithered transitions | Ours is the right family. The orange lines came from **tall thin features in one height map**: the fix is in the art (A15's rules) plus a per-texel decision near the player (plan D), not a new method. |
