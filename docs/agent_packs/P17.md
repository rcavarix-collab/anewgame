# walkgrid helper task P17, `perf_to_svg.py`: a report as a chart (S)

Everything for this task is in this one message (made 2026-09-26). It is task P17 only: ignore any other walkgrid task you may remember from earlier chats.
...
Do task P17 now: the report under `=== FILE: REPORT.md ===` first, then each file under its `=== FILE:` line, and `END OF DELIVERY` at the very end.
